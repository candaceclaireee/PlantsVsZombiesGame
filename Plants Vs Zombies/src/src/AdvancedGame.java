...
import java.util.*;
/**
 * AdvancedGame is a subclass of State and handles the updating and rendering of all entities, timers to generate zombies 
 * and suns and changing delay based on the current number of seconds.
 * 
 * @author Dequito, Christian and Mercado, Candace 
 * @since 2017-07-28
 */
public class AdvancedGame extends State 
{
    private Garden gard;
    ...
    /**
     * Constructor for AdvancedGame. This initializes the world (advanced level) and starts the game timer, timer for 
     * generating zombies and generating sun.
     */
    public AdvancedGame()
    {           
        gard = new Garden("src/res/garden/gardenA.txt");
        
        gameTimer.scheduleAtFixedRate(task1, 0000, 1000); 
	gameTimer.scheduleAtFixedRate(task2, 10000, delay); // GENERATES A ZOMBIES AT INTERVALS/ STARTS AFTER 10S
	gameTimer.scheduleAtFixedRate(task3, 0000, 9000); // GENERATES A SUNS AT EVERY 9
    }
	    ...
	    public void run()
           {
		     ...
		     ZombiesArray.addZombie(zomb);
                 ...
                 {
                     Zombie zomb = new BucketheadZombie(1100, rnd);
                     ZombiesArray.addZombie(zomb);
                 }
